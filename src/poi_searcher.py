    ...
    async def process_poi(self, bbox, poi_type):
        ...
        async with self.locks["results"]:
            ...
            if new_pois:
                ...
                with open(self.config['paths']['output_csv'], 'a', newline='', encoding='utf-8') as f:
                    csv.writer(f).writerows(new_pois)
        ...
        if len(results) > self.config['processing']['threshold']:
            ...
            for quadrant in self.bbox_utils.divide_box(bbox, overlap):
                await self.enqueue_task(quadrant, poi_type)
